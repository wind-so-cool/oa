                    ...
                    <div class="row">
                        <div class="hidden-xs hidden-sm col-md-3">
                            <div class="btn-group">
                                ...
                                <button type="button" class="btn btn-default light">
                                    <i class="fa fa-plus"
                                       onclick="javascript:window.location.href='/oa/claim_voucher/to_add';"></i>
                                </button>
                            </div>
                        </div>
                            ...
                            <tr class="message-unread">
                                ...
                                <td>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_CREATED||cv.status==Contant.CLAIMVOUCHER_BACK}">
                                        <a href="/oa/claim_voucher/to_update?id=${cv.id}">修改</a>
                                    </c:if>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_CREATED||cv.status==Contant.CLAIMVOUCHER_BACK}">
                                        <a href="/oa/claim_voucher/submit?id=${cv.id}">提交</a>
                                    </c:if>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_SUBMIT||cv.status==Contant.CLAIMVOUCHER_RECHECK}">
                                        <a href="/oa/claim_voucher/to_check?id=${cv.id}">审核</a>
                                    </c:if>
                                    <c:if test="${cv.status==Contant.CLAIMVOUCHER_APPROVED}">
                                        <a href="/oa/claim_voucher/to_check?id=${cv.id}">打款</a>
                                    </c:if>
                                    <a href="/oa/claim_voucher/detail?id=${cv.id}">详细信息</a>
                                </td>
                            </tr>
                        </c:forEach>
                        </tbody>
